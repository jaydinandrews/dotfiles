...
awful.keyboard.append_global_keybindings({
    ...
    -- Brightness
    awful.key({}, "XF86MonBrightnessUp", function () awful.spawn("brightnessctl -q s +5%") end,
        {description = "increase brightness", group = "quick"}),
    awful.key({}, "XF86MonBrightnessDown", function () awful.spawn("brightnessctl -q s 5%-") end,
      ...
      grabber =
        awful.keygrabber.run(
          function(_, key, event)
            ...
            if key == "1" then awful.util.spawn(apps.file)
            elseif key == "2" then awful.util.spawn(apps.browser)
            elseif key == "3" then awful.util.spawn(apps.chat)
            elseif key == "4" then awful.util.spawn(apps.music)
            end
